...
Cell Volume::cell( const int i, const int j, const int k ) const {
    ...
    if (!((i >= 0) && (i < n_ - 1))) throw std::invalid_argument("out");
    if (!((j >= 0) && (j < width_ - 1))) throw std::invalid_argument("out");
    if (!((k >= 0) && (k < height_ - 1))) throw std::invalid_argument("out");
	...
	return Cell(rhos, A, A + cell_size_, CellIndices {i, j, k}, this);
}
...
void Volume::Traverse( Ray & ray, std::vector<CellHit> & traversed_cells ) {
	...
	if ( RayBoxIntersection( ray, bounds_, t0, t1 ) ) {
        ...
        t_max += delta *
                 Vector3( // parametrick� sou�adnice prvn�ho vnit�n�ho pr�se��ku s vertik�ln�, resp. horizont�ln� mez� bun�k
                         (ray.direction.x < 0.0f) ? float(n_ - 1 - actual_cell_indices.i) : float(
                                 actual_cell_indices.i + 1),
                         (ray.direction.y < 0.0f) ? float(width_ - 1 - actual_cell_indices.j) : float(
                                 actual_cell_indices.j + 1),
                         (ray.direction.z < 0.0f) ? float(height_ - 1 - actual_cell_indices.k) : float(
                                 actual_cell_indices.k + 1));
        ...
        while (is_inside) {
            ...
            switch (t_max.SmallestComponent(true)) {
                case 0: // tx je minim�ln�
                    ...
                    if ((actual_cell_indices.i < 0) || (actual_cell_indices.i >= n_ - 1))
                        is_inside = false; // nebo is_inside &= ( ( i >= 0 ) && ( i < volume_i ) );
                    break;
...
void Volume::Raycast( Camera & camera, const int samples ) {
    Vector3 light(300.0f, 800.0f, 800.0f);

	printf( "Start ray casting...\n" );

	cv::namedWindow("Render", cv::WINDOW_NORMAL);
	cv::moveWindow("Render", 0, 0);
	cv::Mat image = cv::Mat(cv::Size(camera.width(), camera.height()), CV_32FC3, cv::Scalar(0, 0, 0));
				...
				for ( int xm = 0; xm < samples; ++xm ) {
					...
					for ( int i = 0; i < traversed_cells[tid].size(); ++i ) {
                        ...
                        */
                        ...
                        /*
                        const float value = actual_cell.Integrate(ray, cell_hit);
                        if (value != 0) {
                        ...
                        /*
						// TASK 2: find iso surface
						const float t_hit = actual_cell.FindIsoSurface( ray, cell_hit.t_0, cell_hit.t_1, 0.5f );
						if (t_hit > 0.0f) {
							const Vector3 p = ray.eval( t_hit );
							
							// TODO FOR POINT p: COMPUTE GRADIENT -> NORMAL -> LAMBERT SHADING + OMNI LIGHT -> SURFACE COLOR
                            //https://en.wikipedia.org/wiki/Sobel_operator
                            // dej si to třeba na vzdálenost 0.01 v rámci té jedné buňky
                            // hrozí, že ti ty body spadnou někam mimo tu buňku - buď si je najdeš v té vedlejší buňce, nebo to interpoluješ pouze ze dvou bodů
                            auto n = actual_cell.GradGamma(p);

                            Vector3 L = light - p;
                            L.Normalize();
                            auto LN = L.DotProduct(n);

                            sample_color = Vector3(LN, LN, LN);
							sample_alpha = 1.0f;
							break;
						}
                        */
						...
						actual_cell.FrontToBack(ray, cell_hit, sample_color, sample_alpha);
